# Replica 3 RAFT Log Replication
...
```powershell
$env:REPLICA_ID="replica3"
$env:REPLICA_PORT="3003"
$env:PEERS="http://replica1:3001,http://replica2:3002"
npm start
```
...
```bash
REPLICA_ID=replica3 REPLICA_PORT=3003 PEERS=http://replica1:3001,http://replica2:3002 npm start
```
...
```bash
curl -X POST http://localhost:3003/append-entries \
...
```bash
curl -X POST http://localhost:3003/stroke \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST http://localhost:3003/sync-log \
  -H "Content-Type: application/json" \
  -d '{"fromIndex":0}'
```
